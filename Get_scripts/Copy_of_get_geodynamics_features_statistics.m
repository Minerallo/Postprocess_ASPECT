function [geofeatures] = get_geodynamics_features_statistics(path_model, additional_postprocesses,plumes_depths_tracking,subduction_depths_tracking,plumes_non_adiabatic_tracking_temperature,subduction_non_adiabatic_tracking_temperature,trenches_elevation_threshold,path_model_output,compositional_field_name_of_continents)
...
for i = 1:min_num_files
    ...
    if any(strcmp(additional_postprocesses, 'subduction_and_plume_statistics'))
        ...
        for tt= 1:numel(plumes_depths_tracking)
            ...
            plumes_position = non_adiabT_depths{plumes_depths_tracking(tt)}>plumes_non_adiabatic_tracking_temperature(tt);
            plumes_positions2=1-plumes_position;
            %             boundaries_position3 = ind2rgb(Plate_boundaries2+1, CMap_boundaries);
            cmatrix_plume = contourm(Yeq,Xeq, non_adiabT_depths{plumes_depths_tracking(tt)}, [plumes_non_adiabatic_tracking_temperature(tt) plumes_non_adiabatic_tracking_temperature(tt)], 'r','LineWidth',2);
...
%                   subductions_position3 = ind2rgb(trenches_position2 + 1, CMap_subduction);
             ...
             elseif (subduction_depths_tracking(tt)~=0)
                  ...
                  subductions_position = non_adiabT_depths{subduction_depths_tracking(tt)}<subduction_non_adiabatic_tracking_temperature(tt);
                  subductions_position2=1-subductions_position;
%                   subductions_position3 = ind2rgb(subductions_position2 + 1, CMap_subduction);
                  cmatrix_subduction = contourm(Yeq,Xeq, non_adiabT_depths{subduction_depths_tracking(tt)}, [-90000 subduction_non_adiabatic_tracking_temperature(tt)], 'b','LineWidth',2);
             end  
    ...
    fullFilePath_geofeatures = fullfile(model_output_folder, filename_geofeatures);
...
%     writetable(geofeatures, ['Geofeatures_' model_title '.csv']);
