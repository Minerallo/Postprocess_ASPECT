...
model_names = {'R01f_Rodinia_2GPa_Mantle_C10MPa_f005_LR_SB_f003_rhoCont_3140'};
...
output_directory_name = 'auto_R01fcont3140';
...
%% make a copy job Pv batch to autogenerated folder.sh
file_input_sh = [path_to_postprocess_ASPECT 'Python_paraview/jobparaview_global3D.sh'];
...
fprintf(fid, '##/Applications/MATLAB_R2022b.app/bin/matlab -nodisplay -r "try; run(''/Users/ponsm/Nextcloud/Postdoc_MEET/Modelling/Postprocess_ASPECT/Python_paraview/build_autopostprocess_files.m''); catch;disp(''Error: The MATLAB auto-postprocess files could not be properly built.''); end; quit"\n');
fprintf(fid, '\n');
fprintf(fid, 'rsync -varz -e "ssh -i /Users/ponsm/.ssh/id_rsa_hlrn" /Users/ponsm/Nextcloud/Postdoc_MEET/Modelling/Postprocess_ASPECT/%s/Model_extract_global_3D_auto.py bbkponsm@blogin.hlrn.de:/scratch/usr/bbkponsm/model/globalscale/Potsprocess/postprocess_scripts/%s/.\n',output_directory,output_directory);
fprintf(fid, 'rsync -varz -e "ssh -i /Users/ponsm/.ssh/id_rsa_hlrn" /Users/ponsm/Nextcloud/Postdoc_MEET/Modelling/Postprocess_ASPECT/%s/jobparaview_global3D.sh bbkponsm@blogin.hlrn.de:/scratch/usr/bbkponsm/model/globalscale/Potsprocess/postprocess_scripts/%s/.\n',output_directory,output_directory);
fprintf(fid, '\n');
...
fprintf(fid, ['ssh -i /Users/ponsm/.ssh/id_rsa_hlrn bbkponsm@blogin.hlrn.de ', ...
    '''cd /scratch/usr/bbkponsm/model/globalscale/Potsprocess/postprocess_scripts/%s/; ', ...
    'sbatch -p large96 -t 12:00:00 -N 1 --tasks-per-node 96 -o %%j.output -e %%j.output --mem=747000mb --account bbkponsm --job-name PPv ', ...
    '/scratch/usr/bbkponsm/model/globalscale/Potsprocess/postprocess_scripts/%s/jobparaview_global3D.sh''\n'],output_directory,output_directory);
fprintf(fid, '\n');
fprintf(fid, '# Wait for the job to finish\n');
fprintf(fid, 'while ssh -i /Users/ponsm/.ssh/id_rsa_hlrn bbkponsm@blogin.hlrn.de ''squeue -n PPv'' | grep -q PPv; do\n');
fprintf(fid, '    echo "Job Pv batch is running on the remote server. Waiting..."\n');
